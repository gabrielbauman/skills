---
name: refactoring
description: "Refactor safely: change structure while preserving observable behavior, keep the tests green after every step, and work in small reversible moves instead of one big-bang rewrite. Use whenever restructuring, renaming, extracting, inlining, or otherwise reshaping existing code without changing what it does, when a task needs a preparatory cleanup before the real change, when cleaning up a messy module, or when tempted to rewrite something from scratch. Covers the strict no-behavior-change rule, characterization tests for untested code as the safety net, keeping each step green and revertible, committing the refactor apart from any behavior change, and when not to refactor at all. Pairs with test-behavior, which supplies the green safety net the refactor leans on; with small-prs, whose refactor-first sequencing lands the cleanup before the feature; with atomic-commits, which keeps the refactor in its own commit; and with db-migrations, which applies the same keep-it-working-at-every-step discipline to database schemas."
---
