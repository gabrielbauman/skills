---
name: performance
description: "Optimize measured hotspots against a stated target instead of guessing: profile before changing anything, set a numeric goal so you know when to stop, take the structural wins (N+1 queries, missing indexes, quadratic loops, work in loops that belongs outside) before micro-optimizations, change one thing at a time and re-measure on realistic data, treat caching as a last resort that needs an invalidation story, revert changes that do not move the number, and record before and after figures with the change. Use whenever something is slow, when asked to optimize, speed up, or profile code, when reviewing a change justified by performance, when tempted to add a cache, or when a latency, throughput, memory, or cost target is in play. Pairs with debugging, which brings the same evidence-first loop to correctness, and refactoring, which is what an optimization that does not change behavior must also be."
---
