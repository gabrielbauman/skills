---
name: error-handling
description: "Handle errors and exceptions so failures are loud, actionable, and traceable instead of swallowed. Use whenever writing or reviewing code that can fail: catching exceptions, writing catch/except blocks, designing error messages, adding retries, deciding what to do when a call returns an error, or cleaning up code that hides failures. Covers failing early instead of limping on with bad state, never swallowing an error silently, writing messages that name what was attempted and with which inputs, attaching the underlying cause instead of discarding it, telling recoverable from fatal, catching at the boundary that can act rather than the nearest line, choosing between retry, propagate, and crash, and not leaking secrets or internals to users. Pairs with debugging, whose investigations only work when errors were reported well in the first place, why-comments, for commenting the non-obvious reason behind a catch or rethrow, logging, for logging a caught error once at the boundary that acts instead of at every layer, and secure-coding, which extends the no-leaking rule to the whole trust boundary and adds failing closed when a check cannot complete."
---
