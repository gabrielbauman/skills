---
name: atomic-commits
description: "Decide what each git commit should contain: one self-contained, reviewable change per commit, with a mixed working tree split into an ordered series instead of one everything-commit. Use every time you are about to commit more than one logical change, when asked to split, stage, reorder, or clean up commits, and when planning how to commit as you work. Pairs with scoped-commits (the message), small-prs (what belongs in the branch), and git-recovery (rebase mechanics for reshaping history)."
---
