---
name: db-migrations
description: "Change database schemas without breaking the running system: every migration stays compatible with the code deployed both before and after it (expand and contract), renames and type changes happen as add-backfill-swap rather than in place, backfills run as batched resumable jobs separate from schema DDL, destructive steps ship in their own later deploy, applied migrations are never edited, and lock behavior is checked before DDL touches a hot table. Use whenever writing or reviewing a schema migration, renaming or dropping a column or table, changing a column type or constraint, adding an index to a production database, planning a data backfill, or deciding how migrations run during deploys. Pairs with refactoring, which applies the same keep-it-working-at-every-step discipline to code, and atomic-commits, which keeps each migration step its own change."
---
