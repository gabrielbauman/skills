---
name: test-behavior
description: "Write tests that verify observable behavior rather than pinning the implementation, and clean up suites that test the mechanism instead of the contract. Use whenever writing tests, adding test coverage, fixing a bug (reproduce it in a failing test first), reviewing code that ships with tests, refactoring tested code, or dealing with a brittle, mock-heavy, or flaky suite. Covers asserting on outcomes instead of mock calls and call counts, mocking only at boundaries you do not control, naming tests as behavior claims, boundary and error-path coverage, weak assertions that pass on anything, the red-before-green rule for bug fixes, and handling tests that encode a contract you are deliberately changing. Pairs with adr-workflow, whose tagged tests are the executable spec; with debugging, which produces the reproduction that red-before-green starts from; with branch-review, whose test-coverage charter applies these criteria at pre-merge review; with refactoring, which leans on this green suite as the safety net that proves behavior was preserved; and with flaky-tests, which handles the test that fails intermittently no matter how well it asserts."
---
