---
name: branch-review
description: "Run a pre-merge review of a branch with parallel specialist agents: reviewers with narrow charters (correctness, security, tests, docs, spec conformance) find issues, an adversarial challenger dismisses the false positives, and survivors land in a prioritized report where every HIGH cites a defect or written rule. Depth scales from single pass to full fleet. Use when asked to review a branch, PR, or diff before merge, audit a changeset, or re-review after fixes. Pairs with reviewing-code, the single-reviewer craft this fleet parallelizes."
---
