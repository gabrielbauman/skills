---
name: secure-coding
description: "Write code that holds at the trust boundary: treat every external input as attacker-controlled until validated server-side, build SQL, shell commands, HTML, and paths by parameterization instead of string concatenation, check authorization at every resource access rather than at the menu, keep secrets out of code, logs, URLs, and error messages, use vetted crypto and password hashing instead of homemade, and fail closed when a check cannot complete. Use whenever handling user input or file uploads, building queries or shell commands, adding an endpoint or permission check, storing passwords or tokens, comparing secrets, reviewing code for injection or access-control holes, or reaching for eval, shell=True, or dangerouslySetInnerHTML. Pairs with error-handling, which keeps internals and secrets out of what users see when things fail, and dependencies, which covers the supply-chain half of the attack surface."
---
