...
| [api-design](skills/api-design/) | Design and evolve APIs without breaking deployed consumers: know what counts as a breaking change (renames, type changes, tightened validation, changed error semantics), evolve additively with deprecate-then-remove instead of mutating in place, version as a last resort, and design endpoints around the caller's task with pagination and an error contract from day one. Applies to HTTP/RPC endpoints and a library's exported surface alike. Pairs with db-migrations, the same expand-and-contract discipline applied to schemas. |
...
| [code-comments](skills/code-comments/) | Write code comments that explain why, never what. When a comment is warranted (constraints, workarounds, rejected alternatives) versus noise (narrating the next line, section banners, reviewer-directed remarks), refactoring instead of commenting, docstrings as contracts, TODO hygiene, and what never to delete. |
...
| [reading-code](skills/reading-code/) | Understand unfamiliar code before changing it: match reading depth to the change's blast radius, orient from entry points and build files, trace a real call path instead of trusting the first grep hit, read tests as the spec of intended behavior, use git history to recover why the weird part exists, find the codebase's existing way before inventing a new one, and verify your model of the code before editing on it. Pairs with debugging, the same hypothesis-then-observation discipline aimed at a defect. |
...
| [simplified-technical-english](skills/simplified-technical-english/) | Write procedures, runbooks, and instructional text in Simplified Technical English, adapted from ASD-STE100 for software: imperative one-instruction steps, conditions before commands, active voice with a named agent, one term per thing, 20/25-word sentence caps, notes that never hide requirements, and warnings that state both the action and the consequence. Pairs with writing-docs (what a document contains; this skill governs the sentences). |
...
| [subtractive-engineering](skills/subtractive-engineering/) | Apply a fixed, ordered design process before building anything: question every requirement back to the person who asked for it, delete the part or process outright and add it back only if something breaks, simplify what survives, accelerate the iteration loop, and automate last. The order is the point: optimizing or automating an unquestioned step does the wrong thing faster. Pairs with performance, which governs how to optimize what survives. |
...
| [writing-docs](skills/writing-docs/) | Write documentation for the reader who arrives with a task: lead the README with what the thing is and a copy-pasteable quickstart, keep tutorials, how-to guides, reference, and explanation separate instead of one mixed page, prefer runnable examples over prose, document the sharp edges and failure modes users actually hit, keep docs next to the code so diffs catch them, and fix or delete stale docs on sight. Pairs with humanize (the prose itself) and code-comments (the same judgment inside the code). |
